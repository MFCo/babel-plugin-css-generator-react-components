...
function getAttributes(attList) {
  var attsString = '';
  attList.map(function (atts) {
    if (atts.value.properties === undefined)
      attsString += atts.key.value + ' : ' + atts.value.name + ';';
    else {
      attsString += atts.key.value + '{' + getAttributes(atts.value.properties) + '}';
    }
  })
  return attsString;
}
...
function createCSS(assignment, accessCSSProps, accessName, className, prefix) {
  ...
  assignment.map(function (expression) {
    ...
    nameStyle += '-' + nameComp;
    parent.push(nameStyle);
    var classSelectors = '';
    let tag = nameComp.slice((('STYLED_').length));
    styleMap.set(tag.toLowerCase(), nameStyle);
    styleString += '.' + nameStyle + '{' + getAttributes(accessCSSProps(expression));
    styleString += '}\n';
  ...
  var finalCSS = '';
  postCSS([nestedCSS]).process(styleString).then(result => {
    finalCSS = result.css;
