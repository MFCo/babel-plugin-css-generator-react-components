    ...
    visitor: {
      ...
      JSXElement: {
        enter(path, state) {
          ...
          if ((nameComp.match(/STYLED_*/) !== null) && (styles[styles.length - 1].get(nameComp.slice((('STYLED_').toLocaleLowerCase().length))) !== null)) {
            let element = createJSXElement(nameComp.slice((('STYLED_').length)).toLowerCase(), path.node.openingElement.attributes, path.node.children);
